        ...
        <div class="bg-white shadow-md rounded-lg p-8">
            <form action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="role" name="role" value="{{ $user->role }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                     @error('role')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
                 <div>
                    ...
                    <select id="created_by" name="created_by" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        @foreach($users as $aUser)
                            <option value="{{ $aUser->id }}" {{ $user->created_by == $aUser->id ? 'selected' : '' }}>{{ $aUser->name }}</option>
                        @endforeach
                    </select>
                     @error('created_by')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
